...
import type { CreatePost, PostModel, UpdatePost } from "../models/posts.model.js";
...
export async function getPostById(context: Context) {
    try {
       ...
       const [rows] = await pool.query<PostModel[]>('SELECT * FROM posts WHERE post_id = ?' ,[postId]);
    ...
    try {
        ...
        for (const post of posts) {
            ...
            if (result) {
                ...
                const [data] = await pool.query<PostModel[]>('SELECT * FROM posts WHERE post_id = ?', [postId]);
...
export async function deletePostById(context: Context) {
    try {
        const postId = context.req.param('id');
        const [result] = await pool.query<ResultSetHeader>('DELETE FROM posts WHERE post_id = ?',[postId]);
...
export async function updatePostById(context: Context) {
    ...
    try {
        ...
        const [existingRows] = await pool.query<PostModel[]>('SELECT * FROM posts WHERE post_id = ?', [postId]);
        ...
        if (updatedPost.status && updatedPost.status !== 'Active' && updatedPost.status !== 'Inactive') {
            return context.json({ message: "status must be 'Active' or 'Inactive'" }, 400);
        }

        await pool.query(
            'UPDATE posts SET title = ?, description = ?, status = ? WHERE post_id = ?',
            [updatedPost.title, updatedPost.description, updatedPost.status, postId]
        );
